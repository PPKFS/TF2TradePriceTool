...
{
    class UniqueHatSection : Section
    {
        public override void Sort()
        {
            OrderedList = Items.Keys.OrderBy(t => t.Name).ThenBy(t => t.IsCraftable).ThenBy(t => t.PaintName);
        }

        public override void Print(System.IO.StreamWriter writer)
        {
            writer.WriteLine("**Unique Hats**\n\n");
        ...
        {
            if (item.Quality == Quality.Unique && item.Type == ItemType.Hat)
            {
